...
warmup_steps = int(steps_per_epoch * epochs * 0.15)
stable_steps = int(steps_per_epoch * epochs * 0.05)
decay_steps = int(steps_per_epoch * epochs * 0.8)
...
for epoch in range(epochs):
    for j in range(steps_per_epoch):
        ...
        print(f"Step {j + 1:02d}: LR = {current_lr:.6f}")


# Plot the learning rate schedule
plt.plot(lrs)
plt.title("Warmup → Stable → Cosine Decay LR")
